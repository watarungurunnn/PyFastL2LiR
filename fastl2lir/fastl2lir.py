...
class FastL2LiR(object):
    ...
    def fit(self, X, Y, alpha=1.0, n_feat=0, save_select_feat=False, spatial_norm=None, sample_norm=None, select_sample=None, chunk_size=0, cache_dir='./cache', dtype=np.float64, saveMemory=False):
        ...
        # Chunking
        if save_select_feat:
            assert not saveMemory, f"Save memory and select feat can't be conducted together now."
            assert not sample_norm, f"Sample norm and select feat can't be conducted together now."
        if chunk_size > 0:
        ...
        if reshape_y:
            ...
            if save_select_feat or (sample_norm is not None):
